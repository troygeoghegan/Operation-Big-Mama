...
def draw_crafted_button(screen, rect, text, font, base_color, text_outline_color=None):
    ...
    # Use global finger position if finger is currently pressed (for touch hover feedback)
    if _finger_pressed:
        mx, my = _finger_pos
...
async def _main():
    ...
    try:
        _img_dir = os.path.dirname(os.path.abspath(__file__))
        for fname in ["FML.mov", "FML.MOV", "FML.mp4"]:
            fpath = os.path.join(_img_dir, fname)
            if os.path.exists(fpath):
    ...
    while running:
        ...
        if game_state == GameState.LANDSCAPE_READY:
            ...
            if IS_WEB:
                await play_video_web("FML.mov", allow_skip=False)
